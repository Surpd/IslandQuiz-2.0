-- Stable idempotency key for official content packs.
alter table public.games
  add column if not exists official_content_id text;

create unique index if not exists games_official_content_id_key
  on public.games (official_content_id)
  where official_content_id is not null;

-- The backend validates the complete pack before calling this function. The
-- function is still the transaction boundary: one failing row rolls back the
-- entire batch, while a unique content id is safely reported as skipped.
...
begin
  ...
  for item in select value from jsonb_array_elements(p_games) loop
    ...
    insert into public.games (
      id, kind, data, owner_id, owner_name, visibility, tags,
      official_content_id
    )
    values (
      inserted_id,
    ...
    if found then
      ...
      select g.id into game_id
      from public.games g
      where g.official_content_id = item_content_id;
      content_id := item_content_id;
      status := 'skipped';
    end if;
    return next;
