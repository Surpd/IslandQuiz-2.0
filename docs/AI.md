...
- При manual `type_distribution` backend строго валидирует фактическое распределение; если модель вернула корректный JSON, но нарушила exact counts, full-generation и file-generation выполняют один повторный запрос с corrective-инструкцией и сниженной температурой.
