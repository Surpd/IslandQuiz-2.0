...
- **Статус:** `IN_PROGRESS`; `ae47585` устранил frontend TypeError, но production UI smoke подтвердил, что backend возвращает controlled validation error вместо success payload. Git-history comparison связал это с strict validation gate из `6f6b3d6`; H11 восстанавливает прежний response flow, а canonical backend validation остаётся H8 scope.
