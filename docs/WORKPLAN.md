...
- **Фактический результат:** production Groq response подтвердил root cause: configured `llama-3.3-70b-versatile` больше недоступна текущему key (`model_not_found`). Strict backend validation из `6f6b3d6` маскировал provider error как invalid Quiz/variants, но не был первопричиной. H11 возвращает прежний success flow, использует configurable `GROQ_MODEL`, а frontend guards из `ae47585` остаются границей malformed/empty responses. Successful authenticated smoke, сохранение и play остаются обязательными.
